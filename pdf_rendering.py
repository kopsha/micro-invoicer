...
def render_activity_report(invoice, filename):
    ...
    def render_tasks():
        length = (
            table_column_date_width
            + table_column_client_code_width
            + table_column_description_width
            + table_column_hours_width
            + 38
        )
    ...
    def render_total():
        pdf.set_y(pdf.get_y() + 5)
        pdf.set_x(
            table_column_date_width + table_column_client_code_width + table_column_description_width + 10
        )
...
def render_invoice(invoice, filename):
    ...
    def render_title():
        ...
        pdf.set_x(
            ...
            - pdf.get_string_width(
                f"Seria: {invoice.series} {invoice.number}                            din {invoice.activity.start_date}"
            )
            / 2
        )
        pdf.cell(
            w=0,
            h=5,
            txt=f"Seria: {invoice.series} {invoice.number}                            din {invoice.activity.start_date}",
        )
    ...
    def render_signature_space():
        ...
        pdf.set_x(initialX + 80)
        pdf.multi_cell(
            w=80, h=3, txt="C.I seria SERIE, nr. NUMAR,\nemis de INSTITUTIE\n\nSemnatura", align="C"
        )
